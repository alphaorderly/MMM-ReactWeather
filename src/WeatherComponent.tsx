...
const WeatherComponent: React.FC<WeatherComponentProps> = ({
  ...
  const { today, tomorrow, isLoading, error } = useWeather({
    latitude,
    longitude,
    interval,
  });
  ...
  const scaleMap: Record<string, string> = {
    // Adjusted: significantly smaller main temperature, larger detail (mini/meta-sm) text
    sm: "text-[13px] [&_.today-temp]:text-2xl [&_.mini]:text-[12px] [&_.meta-sm]:text-[12px]",
    md: "text-[14px] [&_.today-temp]:text-3xl [&_.mini]:text-[14px] [&_.meta-sm]:text-[14px]",
    lg: "text-[15px] [&_.today-temp]:text-4xl [&_.mini]:text-[16px] [&_.meta-sm]:text-[16px]",
    xl: "text-[16px] [&_.today-temp]:text-5xl [&_.mini]:text-[18px] [&_.meta-sm]:text-[18px]",
  };
  ...
  const feelsLike =
    today.apparentTemperature != null ? Math.round(today.apparentTemperature) : undefined;
  const tomorrowTempLine = tomorrow
    ? `${Math.round(tomorrow.temperatureMin)}° / ${Math.round(tomorrow.temperatureMax)}°`
    : null;
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-2 w-full">
            <div className="flex flex-wrap items-end gap-4">
              <span className="today-temp font-light tracking-tight tabular-nums">
                {todayTemp}°
              </span>
            </div>
            ...
            <div className="flex flex-col gap-1 mini text-white/70 leading-tight">
              <div className="flex flex-wrap gap-4">
                <span className="meta-sm text-white/60">
                  {Math.round(today.temperatureMin)}° / {Math.round(today.temperatureMax)}
                  °
                </span>
                {feelsLike !== undefined && (
                  <span className="mini text-white/50">Feels {feelsLike}°</span>
                )}
                {today.uvIndex != null && (
                  <span className="mini text-white/50">
              ...
              <div className="flex flex-wrap gap-4">
                {today.windSpeed > 0 && (
                  <span className="flex items-center gap-1 text-white/60">
                    <Wind size={14} />
                    {msToKmh(today.windSpeed).toFixed(0)} km/h
                  </span>
                )}
                {today.windSpeedMax != null && (
                  <span className="flex items-center gap-1 text-white/40">
                    <Wind size={12} />
                    max {msToKmh(today.windSpeedMax).toFixed(0)} km/h
                  </span>
                )}
              </div>
              <div className="flex flex-wrap gap-4">
                ...
                {today.visibility && (
                  <span>Vis {(today.visibility / 1000).toFixed(0)}km</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-col gap-2 w-full">
              <div className="flex flex-wrap items-baseline gap-4">
                <span className="uppercase tracking-wide text-white/55 mini">
                  Tomorrow
                </span>
              ...
              <div className="flex flex-wrap gap-3 text-white/65 mini leading-tight">
                {tomorrow.precipitation > 0 && (
                  <span className="flex items-center gap-1 text-blue-300">
                    <Droplets size={13} />
                    {tomorrow.precipitation.toFixed(1)}mm
                  </span>
                )}
                {tomorrow.windSpeedMax != null && (
                  <span className="flex items-center gap-1">
                    <Wind size={13} />
                    max {msToKmh(tomorrow.windSpeedMax).toFixed(0)} km/h
                  </span>
                )}
                {tomorrow.uvIndexMax != null && (
                  <span>UV Max {Math.round(tomorrow.uvIndexMax)}</span>
                )}
